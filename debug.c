...
/*   Updated: 2025/03/26 06:19:29 by istripol         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    print_philosophers(t_program *program )//, t_philo *philos)
{
    ...
    while (i < program->nb_philos)
    {
		...
		printf("right_fork -> "GREEN"%i (%p) "RESET "| left_fork -> "RED"%i (%p)\n"RESET, program->philos[i].right_fork->id, program->philos[i].right_fork , program->philos[i].left_fork->id, program->philos[i].left_fork);
        printf("-----------\n");
        i++;
    }
